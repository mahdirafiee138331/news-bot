...
def send_telegram_message(text):
    ...
    try:
        ...
        # اگر خطا به خاطر فرمت بود، بدون فرمت ارسال کن
        if "can't parse entities" in e.response.text:
            ...
            logging.error(f"خطای HTTP تلگرام: {e}")
            return False
    except Exception as e:
    ...
    for url in urls:
        ...
        try:
            ...
            for entry in reversed(feed.entries[:15]):
                ...
                escaped_output = html.escape(gemini_output)
                message_part = f"{emojis} <b>{escaped_output.splitlines()[0]}</b>\n\n{escaped_output.splitlines()[1:]}"
                message_part = message_part.replace("\n\n\n", "\n\n")
                message_part += f"\n\n🔗 <a href='{html.escape(entry.link)}'>لینک مقاله اصلی</a>"
                ...
                if send_telegram_message(message_part):
                    ...
                    break # اگر ارسال یک پیام خطا داد، از این سایت خارج شو تا اسپم نشود
        except Exception as e:
